...
export const name = 'generateChapter';
export const description = '根据大纲生成具体章节内容';
export const parameters = {
  type: 'object',
  properties: {
    outline: { type: 'string', description: '章节大纲或概要' },
...
export default async function generateChapterSkill({ outline, genre, style }) {
  const prompt = `你是专业小说创作助手，现在按照以下规则生成小说章节：
...
文风要求：${style || '流畅'}`;
  ...
  return {
    skill: 'generateChapter',
    params: { outline, genre, style },
    prompt
  };
}
